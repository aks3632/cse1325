...
public class Polynomial implements Runnable {
    ...
    /* Modify - searches for the roots of this polynomial,
       storing each one that it finds in the roots ArrayList. */
    public void solve(double min, double max, int nthreads, double slices, double precision) {
        roots.clear();

        int numThreads = nthreads;
        Thread[] threads = new Thread[numThreads];
        double range = (double)(max - min) / numThreads;
    ...
    @Override
    public void run() { // abstract method run() in Runnable
        System.out.print("TrialRun" + " ");
    }
    @Override
